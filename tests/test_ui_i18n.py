...
class UiI18nTests(unittest.TestCase):
    ...
    def test_page_filter_texts_exist(self) -> None:
        ...
        self.assertEqual(text('zh-CN', 'main_tab_query'), '查询台')
        self.assertEqual(text('zh-CN', 'main_tab_results'), '结果与详情')
        self.assertEqual(text('zh-CN', 'main_tab_activity_log'), '活动日志')
