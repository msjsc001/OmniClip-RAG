...
## 2026-07-30 Markdown 热监听事件化与资源生命周期收口

- 桌面端 Markdown `watchdog` 监听现在只在启动时做一次离线期间变更核对，之后完全由文件系统事件触发；没有文件事件时，不允许继续按秒递归遍历整个笔记库。Why：`watchdog` 的价值就是低成本事件通知，如果仍每两秒全库快照，大库空闲时也会持续消耗 CPU、磁盘和临时内存。
- “监听间隔”已拆清为独立的 `watch_debounce_seconds` 静默合并窗口，默认 10 秒；同一文件持续变化会不断顺延，直到停止修改达到该时间才执行一次合并增量更新。旧 `poll_interval_seconds` 只继续服务强制轮询兼容模式。Why：用户编辑过程中每次保存都立刻加载模型，会制造频繁抖动和无意义重复编码。
- 桌面监听线程不再直接导入 Torch 或持有 BGE 模型。每个合并后的增量批次都进入一次性 `--watch-reindex-worker` 子进程，多个库通过进程级锁顺序执行；正常完成、失败或用户停止后都必须等待并回收子进程。Why：PyTorch/CUDA 原生分配器不保证在长寿命 GUI 进程里把 Commit 完整还给 Windows，进程退出才是最可靠的资源边界。
- 已知的 Logseq/同步工具生成目录 `.stversions`、`logseq/bak`、`logseq/.recycle`、`logseq/.tine-trash` 不再进入预检、全量建库、启动核对或实时事件增量链。Why：这些内容不是用户的正式笔记真相源，却会在后台自动变化，过去会让用户误以为“没有改文件，GPU 仍持续工作”。
