...
>
> For the ordinary Windows desktop app, download the release asset whose name contains `WIN-EXE`, such as `OmniClipRAG-v0.4.13-WIN-EXE.zip`.
...
> Download `OmniClipRAG-MCP-v0.4.13-win64.zip` for manual `stdio` setup or `omniclip-rag-mcp-win-x64-v0.4.13.mcpb` for the official MCP Registry / MCPB path.
...
- `OmniClipRAG-MCP-v0.4.13-win64.zip` for manual file-based setup
- `omniclip-rag-mcp-win-x64-v0.4.13.mcpb` for the official MCP Registry and MCPB-aware clients
...
- If your client supports MCPB installation, prefer the Release asset: `omniclip-rag-mcp-win-x64-v0.4.13.mcpb`
...
- `Command`: the full path to `OmniClipRAG-MCP.exe` (e.g. `D:\Apps\OmniClip RAG\dist\OmniClipRAG-MCP-v0.4.13\OmniClipRAG-MCP.exe`)
...
### V0.4.13 Key Updates
`v0.4.13` makes Markdown live watch genuinely event-driven and moves each incremental parse, embedding, and CUDA workload into a disposable child process.
- ⚡ **Idle watch no longer rescans every vault**: after startup reconciliation, `watchdog` responds only to real filesystem changes.
- ⏱️ **Users can set the quiet period after editing**: the 10-second default merges repeated saves into one incremental update.
- 🧹 **Large resources are released after every batch**: parsing, embedding, and vector writes finish in a disposable process so Windows can reclaim Commit, RAM, and VRAM.
- 🧵 **Watched vaults do not load duplicate models concurrently**: semantic update batches are serialized to reduce GPU and Windows Commit peaks.
- 🚫 **Generated Logseq history and recovery trees are ignored**: `.stversions`, backups, recycle trees, and conflict-trash files no longer trigger pointless updates.

### V0.4.12 Key Updates
`v0.4.12` replaces the universal Reranker memory floor with a model-aware live resource estimate and makes query progress visible directly beside the Search action.
- 🧠 **Auto now evaluates the workload instead of a universal 10 GiB floor**: the Reranker decision uses the local model weight, batch settings, Windows Commit headroom, available physical memory, and free CUDA memory.
- 🎯 **The compact query-state button now names the active stage**: preparing, lexical recall, semantic recall, fusion, reranking, result finalization, context packing, and fallback stages are visible with progress.
- 🧭 **Query failures remain visible**: a failed query no longer falls back to showing the previous completed state.
- 📐 **Numeric query controls stay attached to their labels**: score threshold and result-limit fields no longer drift away from their explanatory text on wide windows.
- 🧹 **Disposable query children are explicitly reaped**: normal completion, cancellation, and failure paths wait for process termination instead of leaving query workers behind.
- 🛑 **Multi-vault live watch stops reliably**: global stop covers every actual watcher, each vault shows a non-repeatable stopping state, and scans plus incremental vector writes honor safe cooperative cancellation.
- 🧵 **PDF/Tika directory actions stay responsive**: completion-time state reloads and indexed-source summaries run through a coalesced background refresh, and stale results cannot overwrite newer directory settings.
...
- 🛑 **Queries stay disabled while startup Runtime detection is active**: both buttons and the execution guard remain blocked until the detection result is available.
- 🔄 **Safe startup no longer leaves a stale repair warning**: the isolated status-only Runtime probe still runs in safe mode, and its exact result is applied to the Start and Runtime pages.
- 🧱 **Semantic queries now run in a disposable child process**: native PyTorch, SentenceTransformer, or CrossEncoder access violations can no longer terminate the main Qt interface.
- 🛟 **A crashed semantic query gets one clean lexical recovery attempt**: the UI stays alive and can still return useful results with a visible degradation warning.
- 🧠 **Multi-vault model reuse remains bounded to one query process**: selected vaults share the same embedding/reranker resources during a query, and the operating system reclaims them when the child exits.

### V0.4.9 Key Updates
`v0.4.9` completes the startup-responsiveness work with an isolated Runtime probe, truthful detecting states, staggered background refreshes, and a new high-contrast flat application icon.
- ⚡ **Runtime detection starts automatically after first paint**: the Start and Runtime pages immediately show a detecting state and update themselves without requiring a manual refresh.
- 🧵 **Heavy startup work is isolated from the GUI process**: Runtime validation and the initial workspace snapshot run in disposable child processes, while Markdown, extension, and Tika refreshes start in a controlled sequence.
- 🪶 **First paint avoids unnecessary parser and ML initialization**: the full Tika format catalog is parsed only when the format picker is opened, and lightweight status paths no longer probe vector dependencies.
- 🧭 **Status labels are more truthful**: an installed but not-yet-verified Runtime is no longer reported as missing, while enabled PDF/Tika sources distinguish unbuilt state from pending verification.
- ✨ **The application icon is now a simpler flat mark**: the new balanced ring-and-spark design remains recognizable at Windows thumbnail sizes.
...
- 🚀 **Packaged startup is much less likely to freeze on first paint**: heavyweight Runtime-management refreshes and extension-source summary rebuilds now move behind the initial window draw instead of blocking the GUI thread before the shell becomes visible.
- 🧭 **PDF/Tika source rows finally have a real remove flow**: the danger action no longer stops at “clear index”; it now removes the source directory from the extension registry/UI after cleaning its matching extension index/state data.
- 🧹 **Extension-side cleanup is now complete but still safe**: removing a PDF/Tika source clears local extension state and index traces for that source, while still never deleting the original external folder.
- 🛑 **Cancelled delete tasks keep their source directories intact**: the UI no longer risks dropping a source directory from extension settings when the underlying delete task did not actually complete.
- 🧪 **The local release line is internally consistent again**: package metadata, setup examples, release notes, and MCP-facing paths now all line up on the `v0.4.8` version line.
