...
> **MCP 快速入口**
>
> 普通 Windows 桌面用户请下载文件名带有 `WIN-EXE` 的发行包，例如 `OmniClipRAG-v0.4.13-WIN-EXE.zip`。
...
- `OmniClipRAG-MCP-v0.4.13-win64.zip`：给手动配置 `stdio` 的用户
- `omniclip-rag-mcp-win-x64-v0.4.13.mcpb`：给官方 MCP Registry 与支持 MCPB 的客户端
...
### V0.4.13 重点更新
`v0.4.13` 把 Markdown 热监听改成真正的事件驱动，并把每次增量解析、向量化和 CUDA 工作放进用完即退出的独立进程。
- ⚡ **空闲时不再反复扫描整个笔记库**：启动核对完成后，`watchdog` 只响应实际文件变化。
- ⏱️ **可设置停止编辑后的等待时间**：默认等待 10 秒，把连续保存合并成一次增量更新，避免边写边反复索引。
- 🧹 **增量更新结束即释放大资源**：解析、Embedding 和向量写入由一次性子进程完成，结束或停止后由 Windows 回收 Commit、内存和显存。
- 🧵 **多个笔记库不会同时加载重复模型**：增量语义任务串行执行，降低 GPU 与 Windows Commit 峰值。
- 🚫 **忽略 Logseq 生成的历史与回收目录**：`.stversions`、备份、回收站和冲突垃圾文件不再触发无意义更新。
...
- 🧠 **Auto 模式继续使用 CUDA 语义召回**：Commit 余量不足时只跳过可选的 Reranker，不再关闭向量检索，也不会丢掉整次语义查询。
- 🛟 **原生查询故障仍优先保留语义能力**：新的独立进程会先尝试 CPU 语义召回并跳过 Reranker，只有这一步也失败才最终退回纯字面检索。
- 🧹 **查询子进程退出时不再把大型 CUDA 模型搬回 CPU**：结果安全写回后由 Windows 随进程退出回收资源。
- 🧭 **查询台直接告诉用户如何处理**：关闭高内存程序、保持 Auto、启用 Windows 自动管理分页文件或适当增大分页文件，必要时重启；这种情况无需重装 Runtime 或模型。

### V0.4.10 重点更新
`v0.4.10` 是一个候选稳定性版本，重点是让 Runtime 检测和语义模型故障都无法拖垮可交互的桌面主进程。
- 🛑 **启动 Runtime 检测期间禁止查询**：查询按钮和执行入口会同时保持阻塞，直到检测结果返回。
- 🔄 **安全启动不再留下过期的“待修复”提示**：safe-mode 仍会运行独立的轻量 Runtime 状态探测，并把准确结果同步到开始页和 Runtime 页。
- 🧱 **语义查询整体迁移到一次性子进程**：PyTorch、SentenceTransformer 或 CrossEncoder 的 Windows 原生访问冲突不会再导致 Qt 主界面退出。
- 🛟 **语义子进程崩溃后自动进行一次干净的字面检索恢复**：主界面保持可用，并用明确提示说明本次结果已降级。
- 🧠 **多笔记库仍在单次查询内共享模型**：同一查询的多个库复用 embedding/reranker，查询子进程结束后由操作系统完整回收资源。
