...
> 理论上如果你把与 AI 聊天或任意文本内容保存到你的笔记库下，它也是个变相的成为了一个无限记忆的语义索引库。
...
这一轮主要把“方寸引”命名、GPU 自动能力、建库时间反馈和查询审阅体验一起补齐了。

- 中文名正式改为 **方寸引**，桌面标题、界面文案和中英文说明文档已统一同步。
- 默认设备策略改为 `auto`，当本机 PyTorch 运行时真实支持 NVIDIA CUDA 时，会自然切到 GPU；否则安全回退到 CPU。
- 预检查和建库 ETA 现在会参考当前工作区最近几次真实建库记录，不再只靠静态公式估算。
- 全量建库的暂停 / 继续 / 取消链路更完整了，向量阶段也改成了更容易中断的批次写入。
- 查询结果现在支持相关性过滤、逐条勾选是否进入上下文包、命中原因与节选查看，手动审阅更稳。
- 中文 Windows 下复制上下文包时的 `gbk` 编码问题已经修复，不会再因为特殊符号导致复制失败。
- 文档已经更新到当前真实产品形态，包括共享数据区 / 笔记库工作区、方寸引命名和设备自动选择规则。
