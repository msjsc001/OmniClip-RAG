...
> 从 `v0.4.6` 起，MCP 线在继续提供手动 ZIP 的同时，也已经把共享环境根、诚实降级提示、标准 MCPB 发布物和仓库内可复现构建链全部收紧到同一版发布面。
...
从 `v0.4.6` 开始，MCP 这条线会同时提供两种分发形态：
- `OmniClipRAG-MCP-v0.4.6-win64.zip`：给手动配置 `stdio` 的用户
- `omniclip-rag-mcp-win-x64-v0.4.6.mcpb`：给官方 MCP Registry 与支持 MCPB 的客户端
...
```json
{
  "mcpServers": {
    "omniclip-rag": {
      "transport": "stdio",
      "command": "D:\\Apps\\OmniClip RAG\\dist\\OmniClipRAG-MCP-v0.4.6\\OmniClipRAG-MCP.exe",
      "args": []
    }
  }
}
...
如果你希望 AI 不是每次都等你提醒才去用知识库，而是更像“天然知道该先检索再对话”，下面这两组提示词很适合长期固定使用。

#### 给已经接入 MCP 的 AI

当 AI 可以自己调用 `omniclip.search` 时，可以直接给它这段：

```text
之后我们聊到什么，请你都先围绕当前问题自行搜索我的本地知识库，再结合检索结果与我的知识库来和我聊天。你可以把这些内容理解成我的知识边界，只是我自己不一定每次都能记得很清楚。

因为我的知识库可能很大，里面可能有几十万个切片，请你不要过度发散搜索，只围绕当前话题所需的信息进行检索和使用。

后续聊天里如果你还需要更多我的知识背景，也继续用同样的方式：先搜索本地知识库，再继续和我对话。
```

#### 给没有 MCP 的网页 AI

当你在普通网页端聊天、AI 没法直接调用 MCP 时，可以给它这段：

```text
之后我们聊到什么，请你都先判断当前问题需要我本地知识库里的哪些信息，然后直接告诉我你需要搜索的关键词或词句。我会用本地 RAG 工具手动搜索，再把检索结果发给你。

因为当前这个网页聊天没有 MCP 能力，所以请你模拟自己具备 MCP 能力：主动告诉我你需要搜索什么，等我把检索切片返回给你后，再结合这些切片与我的知识库来继续和我聊天。你可以把这些内容理解成我的知识边界，只是我自己不一定每次都能记得很清楚。

因为我的知识库可能很大，里面可能有几十万个切片，请你不要过度发散搜索，只围绕当前话题所需的信息进行检索和使用。

后续聊天里如果你还需要更多我的知识背景，也继续用同样的方式来进行。
```
