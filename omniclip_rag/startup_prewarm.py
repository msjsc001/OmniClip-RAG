...
def evaluate_startup_status_refresh(
    *,
    ...
    # Safe startup suppresses semantic prewarming, not the isolated status
    # probe. Skipping this probe leaves the Start page on a stale
    # "runtime needs repair" snapshot until the user refreshes manually.
    del safe_mode
    if not runtime_complete:
        return PrewarmDecision(False, 'runtime-incomplete', memory)
    if memory is None:
