...
class RuntimeInstallDialog(QtWidgets.QDialog):
    def __init__(
        ...
        *,
        ...
        self.setWindowTitle(self._tr('runtime_manual_dialog_window_title', component=self._value('component_name')))
        self.setModal(True)
        ...
        title = QtWidgets.QLabel(self._tr('runtime_manual_dialog_title', component=self._value('component_name')), header_card)
        title.setProperty('role', 'title')
        header_layout.addWidget(title)
        subtitle = QtWidgets.QLabel(
            self._tr(
                'runtime_manual_dialog_subtitle',
                component=self._value('component_name'),
                description=self._value('component_description'),
                download_usage=self._value('download_usage'),
                disk_usage=self._value('disk_usage'),
            ),
            header_card,
        )
        ...
        self._add_source_card(
            body_layout,
            title=self._tr('runtime_manual_dialog_official_title'),
            url=self._value('official_url'),
            command=self._value('official_install_command'),
        )
        self._add_source_card(
            body_layout,
            title=self._tr('runtime_manual_dialog_mirror_title'),
            url=self._value('mirror_url'),
            command=self._value('mirror_install_command'),
        )
    ...
    def _add_target_card(self, layout: QtWidgets.QVBoxLayout) -> None:
        card, card_layout = self._make_card(self._tr('runtime_manual_dialog_target_title'))
        layout.addWidget(card)
        label = QtWidgets.QLabel(self._tr('runtime_manual_dialog_target_body', component=self._value('component_name')), card)
        label.setWordWrap(True)
        label.setTextInteractionFlags(QtCore.Qt.TextInteractionFlag.TextSelectableByMouse)
        card_layout.addWidget(label)
        card_layout.addWidget(self._make_code_box(self._value('runtime_dir'), minimum_lines=3))
        actions = QtWidgets.QHBoxLayout()
        actions.setSpacing(8)
        card_layout.addLayout(actions)
        copy_button = QtWidgets.QPushButton(self._tr('copy_path_button'), card)
        copy_button.setProperty('variant', 'secondary')
        copy_button.clicked.connect(lambda: self._copy_text(self._value('runtime_dir')))
    ...
    def _add_source_card(self, layout: QtWidgets.QVBoxLayout, *, title: str, url: str, command: str) -> None:
        ...
        layout.addWidget(card)
        url_label = QtWidgets.QLabel(self._tr('runtime_manual_dialog_source_url'), card)
        ...
        command_label = QtWidgets.QLabel(self._tr('runtime_manual_dialog_source_command'), card)
    ...
    def _add_finish_card(self, layout: QtWidgets.QVBoxLayout) -> None:
        card, card_layout = self._make_card(self._tr('runtime_manual_dialog_finish_title'))
        layout.addWidget(card)
        label = QtWidgets.QLabel(self._tr('runtime_manual_dialog_finish_body', component=self._value('component_name')), card)
        label.setWordWrap(True)
        label.setTextInteractionFlags(QtCore.Qt.TextInteractionFlag.TextSelectableByMouse)
        card_layout.addWidget(label)
    ...
    def _open_target_dir(self) -> None:
        target_dir = self._value('runtime_dir')
        if not target_dir:
            return
